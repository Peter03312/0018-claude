...
结论产生后，结论区会出现 **复制复核链接** 入口。链接只携带当前三项选择：

```
http://<host>/?face=gum&top=R&tip=U
```

- `face=design|gum`（观察面）、`top=U|R|D|L`（邮票上边方向）、`tip=U|R|D|L`（水印尖端方向）；
- 编码固定按 `face → top → tip` 规范顺序输出，只含这三项；解析与参数出现顺序无关（乱序亦可恢复）；
- 同事打开链接即恢复相同选择，继续调用同一归一函数，看到相同步骤图与目录视向结论；
- 链接中某字段取值未知时，**仅忽略该字段**并在对应控件旁提示“链接中的取值无法识别，请重新选择”，其他合法字段保留；在该控件重新选择后提示消失；
- 浏览器拒绝剪贴板访问时，在按钮旁反馈失败（附可手动复制的链接），不改变当前结论；
- 不带任何查询参数的访问与原有手动校核流程完全一致。
...
- `src/domain/reviewLink.test.ts`：复核链接 2×4×4=32 个取值组合的 encode→parse 往返一致性（并经原归一函数交叉校验）、乱序参数解析、未知取值只忽略该字段、链接只含三项。
- `src/ReviewLink.test.tsx`：合法链接首屏恢复选择/步骤图/结论、未知值就近提示且重选后消失、复制成功提示随旧结论同步消失、链接随新结论更新、剪贴板被拒/不可用时的失败反馈。
- `e2e/flow.spec.ts`（Playwright）：真实浏览器中验证完整选择流程与上述对照；以背胶面·上边朝右·尖端朝上实例验证“复制复核链接 → 同事新页面打开即见相同选择、步骤图和结论”，以及未知值链接的就近提示。
